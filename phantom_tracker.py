...
that multiple background checkpoint threads (and, on CryptoHybrid, the BTC and
...
# Verdict thresholds (GBP) -- classified on the 1hr post-decision window. PER-MARKET
# (System 5 Review desk-wide, 18 Jul 2026): a single threshold cannot serve BTC (~£47k)
# and ETH (~£1.3k) -- a 10x scale gap. Each threshold ~= that market's median absolute
# 1hr move, restoring the desk-standard 40-60% classification rate.
#
# BTC 40 -> 14 (19 Jul 2026): the 18 Jul momentum-scalping rebuild produces much smaller
# BTC moves than the old LONG_ONLY trend strategy (new-regime median |1hr move| £13.90 vs
# £73.20 old). Threshold 40 was calibrated on the old-strategy-dominated blend (median
# £38.40) and under-classified the new regime (~23%% on 18 Jul+ rows). 14 ~= the new-regime
# median -> ~50%% classification. Pre-18 Jul rows archived to phantom_pre_scalping_archive.csv.
# ETH stays 4 -- only 1 new-regime ETH row; recalibrate when 30+ exist (Gaius monthly check).
VERDICT_THRESHOLD = {"BTC": 14.0, "ETH": 4.0}
VERDICT_THRESHOLD_DEFAULT = 10.0


def _threshold_for(market):
    """Per-market verdict threshold (GBP); falls back to the default for unknown markets."""
    return VERDICT_THRESHOLD.get(str(market).upper(), VERDICT_THRESHOLD_DEFAULT)
...
def _session_from_utc(now_utc):
    ...
    own (e.g. 24/7 CryptoHybrid). Systems with real sessions pass their own."""
    try:
        h = now_utc.hour
    except Exception:
        return ''
...
def _calculate_verdict(direction_blocked, price_at_decision, price_1hr, market=None):
    """
    Per-market threshold (System 5 Review). CORRECT: price moved > threshold AGAINST our
    blocked entry direction (right to stay out — would have been a loss). WRONG: moved >
    threshold IN FAVOUR (wrong to stay out — would have profited). NEUTRAL: smaller move.
    """
    ...
    threshold = _threshold_for(market)
    if move > threshold:
                ...
                if row_index < len(rows):
                    ...
                    if price_col == 'price_1hr':
                        rows[row_index]['verdict'] = _calculate_verdict(
                            direction_blocked, price_at_decision, price, market
                        )
...
def _dedup_flag(market, direction_blocked):
    ...
    and in the FALSE case advance the per-market anchor. Per-instrument (BTC/ETH separate).
    Never raises; on any error returns 'FALSE' (fail-open so a genuine signal is never
    suppressed)."""
        ...
        if row.get('price_1hr') and row.get('verdict') == 'PENDING':
            try:
                ...
                row['verdict'] = _calculate_verdict(
                    direction, price_at_decision, price_1hr, market
                )
                changed = True
                logger.info(
